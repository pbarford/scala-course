...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = google.map(x => TweetReader.allTweets.filter(tweet => tweet.text.contains(x))).reduceLeft(_.union(_))
  lazy val appleTweets: TweetSet = apple.map(x => TweetReader.allTweets.filter(tweet => tweet.text.contains(x))).reduceLeft(_.union(_))
